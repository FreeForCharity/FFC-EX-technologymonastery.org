...
//
// This is strictly MORE data than the previous model, where the GTM/GA4
// scripts did not load at all until a visitor granted analytics consent —
...
// permissive default: Microsoft Clarity would load only on an explicit
// analytics grant and Meta Pixel only on an explicit marketing grant —
// everywhere, including the US. See components/CookieConsent.tsx (both
// are currently unprovisioned placeholders on this site, so both loaders
// are inert).
//
 ...
 *
 * Emitted into <head> in the root layout, ahead of the GoogleTagManager
 * component. Two `consent default` calls, in Google's documented order:
 * the region-scoped denial first, then the global grant. Region-specific
 ...
 *
 * Matching the canary reference (FFC-EX-canary), `wait_for_update` is set
 * on BOTH default calls, not just the region-scoped denial: GTM loads from
 * the layout here, so without a wait on the unscoped grant it could
 * initialise under the granted default before React hydrates and restores
 * a returning non-EEA visitor's stored DECLINE. The wait gives the
 * stored-choice restore a window before tags evaluate consent, in every
 * region.
 *
